...
What the agent has actually done is discovered, through exhaustive probing, that the service is down -- and told no one, learned nothing globally, and spent its own budget generating the news. A human operator in the same situation trips a breaker within minutes: after the third identical failure, the service is marked down, alerts fire, and everyone stops paying it attention until it recovers.
